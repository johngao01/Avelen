...
ERROR_NOTIFY_STATE_FILE = LOGS_DIR / 'error_notify_state.json'
ERROR_NOTIFY_LOCK_FILE = LOGS_DIR / 'error_notify_state.lock'
LOGS_DIR.mkdir(exist_ok=True)
TELEGRAM_LOCAL_MODE = os.getenv('TELEGRAM_LOCAL_MODE', '1').strip().lower() not in {'0', 'false', 'no', 'off'}
TELEGRAM_BASE_URL = os.getenv('TELEGRAM_BASE_URL', 'http://localhost:8081/bot')
TELEGRAM_BASE_FILE_URL = os.getenv('TELEGRAM_BASE_FILE_URL', 'http://localhost:8081/file/bot')
...
INSTAGRAM_COOKIE_PATHS = tuple(
    COOKIES_DIR / cookie_file
    for cookie_file in INSTAGRAM_CONFIG['cookie_files']
)
